...
binary_tuning <- function(
  ...
  if (!is.finite(var_lp_unit) || var_lp_unit <= 0) {
    stop(
      "Estimated Var(LP) at beta_signal = 1 is not a positive finite ",
      "number (got ",
      format(var_lp_unit),
  ...
  find_mu <- function(lp_scaled) {
    ...
    hi <- mu0 + 5 * s
    for (k in seq_len(20)) {
      if (mean(stats::plogis(lo + lp_scaled)) < target_prevalence) {
        break
      }
      lo <- lo - 2^k * s
    }
    for (k in seq_len(20)) {
      if (mean(stats::plogis(hi + lp_scaled)) > target_prevalence) {
        break
      }
      hi <- hi + 2^k * s
    }
  ...
  if (
    abs(prevalence_achieved - target_prevalence) > tolerance ||
...
check_generator_defaults <- function() {
  ...
  for (a in shared) {
    ...
    if (!isTRUE(all.equal(tv, gv))) {
      stop(sprintf(
        paste0(
          "Default for '%s' differs between binary_tuning() (%s) and ",
...
quickcstat <- function(y, pred) {
  ...
  if (length(casepred) == 0 || length(conpred) == 0) {
    return(NA_real_)
  }
