...
#
# Simulation-based bisection on beta (kept)
#   Simulate lp_unit (LP at beta = 1) once, rescale it across bisection
#   iterations (exact: generate_linear_predictor is linear in beta_signal),
#   solve mu for prevalence at each step, and bisect on beta for the AUC.
#   This core is correct for every complexity level.
#
# Current revision: fixing prevalence/AUC RECOVERY
# ------------------------------------------------
# Symptom
#   The tuned (beta, mu) reproduced the target prevalence and AUC inside the
#   tuner, yet a downstream call to generate_binary_data() returned a much
#   higher prevalence and AUC.
#
# Root cause (two compounding issues)
#   1. SILENT DEFAULT MISMATCH. binary_tuning() defaulted correlation = 0 while
#      generate_binary_data() defaults correlation = 0.3. A user tuning with
#      defaults and generating with defaults therefore got correlated
#      predictors the tuner never modelled. With S signal predictors the
#      signal variance is inflated from S to S + S(S-1)*rho, so the realised
#      AUC and prevalence both overshoot (e.g. AUC 0.75 -> 0.83,
#      prevalence 0.20 -> 0.24). The tuner is fully capable of handling any
#      correlation -- it simply needs to be TOLD the same value the generator
#      will use.
#   2. SELF-FULFILLING VALIDATION. prevalence_achieved / auc_achieved were
#      computed by re-using the tuner's own lp_unit and pre-drawn U_binom, so
#      they always matched the target even when the real generator diverged.
#      The function could not detect its own mismatch.
#
# Fixes
#   (a) The predictor-generation defaults (correlation, distribution,
#       predictor_type, binary_prevalence) are now aligned with
#       generate_binary_data(). A startup guard (check_generator_defaults())
#       errors out if those shared defaults ever drift apart again, so this
#       class of bug cannot return silently.
#   (b) The final validation now performs a genuine, independent
#       generate_binary_data() call with the resolved settings and scores the
#       oracle linear predictor with a full-sample c-statistic. The returned
#       prevalence_achieved / auc_achieved therefore describe what the USER
#       will actually obtain, and a warning is raised if either deviates from
#       its target beyond a tolerance.
#
# NOTE TO CALLERS
#   Whatever predictor settings you pass to generate_binary_data() (complexity,
#   nonlinear_strength, correlation, distribution, predictor_type,
#   binary_prevalence) MUST be passed identically to binary_tuning(). The
#   tuned (beta, mu) are only valid for the exact generating configuration they
#   were tuned under.
# =============================================================================
