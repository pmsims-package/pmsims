...
survival_cindex <- function(data, model){
  ...
  if (class(try(concordancefit(y_surv, -1*y_hat), silent=TRUE))[1]=="try-error"){
    cindex = NaN 
  }else{
    if (is.null(concordancefit(y_surv, -1*y_hat)$concordance)) {
      cindex= NaN
    }else{
      cindex=concordancefit(y_surv, -1*y_hat)$concordance}
  }
  return(cindex)
}
