...
survival_tuning <- function(
  ...
  X_unit <- generate_predictors(
    n = n_sim, n_signal_parameters = n_signal, noise_parameters = noise,
    complexity = complexity, predictor_type = predictor_type,
    binary_prevalence = binary_prevalence, correlation = correlation,
    distribution = distribution
  )
  lp_unit <- generate_linear_predictor(
    X = X_unit, n_signal_parameters = n_signal, noise_parameters = noise,
    intercept = 0, beta_signal = 1, complexity = complexity,
    nonlinear_strength = nonlinear_strength
  )
  var_lp_unit <- stats::var(lp_unit)
  if (!is.finite(var_lp_unit) || var_lp_unit <= 0)
    stop("Var(LP) at beta = 1 is not positive and finite -- the linear ",
  ...
  cindex_at_beta <- function(beta) {
    ...
    survival::concordance(survival::Surv(tm, ev) ~ lp, reverse = TRUE)$concordance
  }
  
  # ---- Step 3: bisect beta to hit the target C-index -------------------------
  lo <- beta_interval[1]; hi <- beta_interval[2]
  for (k in seq_len(8)) {
    if (cindex_at_beta(hi) >= target_performance) break
    hi <- hi * 2
  }
  ...
  for (i in seq_len(60)) {
    mid <- (lo + hi) / 2
    if (cindex_at_beta(mid) < target_performance) lo <- mid else hi <- mid
  ...
  val <- generate_survival_data(
    n = n_validate, n_signal_parameters = n_signal, noise_parameters = noise,
    beta_signal = beta_opt, baseline_hazard = lambda_opt,
    censoring_rate = 1 - target_prevalence,
    complexity = complexity, nonlinear_strength = nonlinear_strength,
    predictor_type = predictor_type, binary_prevalence = binary_prevalence,
  ...
  lpv <- generate_linear_predictor(
    X = Xv, n_signal_parameters = n_signal, noise_parameters = noise,
    intercept = 0, beta_signal = beta_opt, complexity = complexity,
    nonlinear_strength = nonlinear_strength
  )
  event_rate <- mean(val$event)
  cindex     <- survival::concordance(
    survival::Surv(val$time, val$event) ~ lpv, reverse = TRUE)$concordance
  
  if (abs(event_rate - target_prevalence) > tolerance ||
    ...
    warning(sprintf(
      ...
      tolerance, target_prevalence, event_rate, target_performance, cindex))
  }
  
  c(lambda_opt  = lambda_opt,
