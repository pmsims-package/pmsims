...
survival_tuning <- function(
  ...
  obj_fun <- function(x) {
    ...
    (event_rate_achieved - target_prevalence)^2 + (cindex_achieved - target_performance)^2
  }
  ...
  censoring_time_final <- rep(quantile(event_time_final, target_prevalence), N_sim_final)
