...
validate_complexity <- function(complexity) {
  if (
    length(complexity) != 1L ||
      !is.numeric(complexity) ||
...
validate_data_control <- function(data_control, complexity) {
  ...
  if (!is.null(ns)) {
    if (!is.numeric(ns) || length(ns) != 1L || ns < 0 || ns >= 1) {
      stop(
        "`data_control$nonlinear_strength` must be a single value in [0, 1).",
        call. = FALSE
      )
    }
    if (complexity %in% c(1, 4)) {
      warning(
        "`nonlinear_strength` is ignored for complexity ",
        complexity,
  ...
  if (!is.null(corr)) {
    if (!is.numeric(corr) || length(corr) != 1L || corr < -1 || corr > 1) {
      stop(
        "`data_control$correlation` must be a single value in [-1, 1].",
        call. = FALSE
      )
    }
  }
  ...
  if (!is.null(pd)) {
    if (!is.character(pd) || length(pd) != 1L || !(pd %in% allowed_dist)) {
      stop(
        "`data_control$predictor_distribution` must be one of: ",
  ...
  if (identical(pd, "binary")) {
    ...
    if (!is.numeric(bpp) || length(bpp) != 1L || bpp <= 0 || bpp >= 1) {
      stop(
        "`data_control$binary_predictor_prevalence` must be in (0, 1).",
        call. = FALSE
      )
    }
    if (complexity %in% c(2, 3)) {
      stop(
        "Binary predictors are incompatible with complexity ",
        complexity,
