...
[`set.seed`](https://rdrr.io/r/base/Random.html)`(``123``)`` `` ``binary_example`` ``<-`` `[`simulate_binary`](https://pmsims-package.github.io/pmsims/reference/simulate_binary.md)`(`` `` signal_parameters ``=`` ``20``,`` `` noise_parameters ``=`` ``0``,`` `` predictor_type ``=`` ``"continuous"``,`` `` binary_predictor_prevalence ``=`` ``NULL``,`` `` outcome_prevalence ``=`` ``0.30``,`` `` maximum_achievable_cstatistic ``=`` ``0.80``,`` `` model ``=`` ``"glm"``,`` `` metric ``=`` ``"calibration_slope"``,`` `` target_performance ``=`` ``0.85``,`` `` n_reps_total ``=`` ``1000``,`` `` mean_or_assurance ``=`` ``"assurance"`` ``)`` `` ``binary_example`
