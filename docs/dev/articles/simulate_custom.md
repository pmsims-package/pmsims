...
[`set.seed`](https://rdrr.io/r/base/Random.html)`(``1234``)`` ``result`` ``<-`` `[`simulate_custom`](https://pmsims-package.github.io/pmsims/dev/reference/simulate_custom.md)`(`` `` data_function ``=`` ``my_data_generator``,`` `` model_function ``=`` ``my_model_function``,`` `` metric_function ``=`` ``my_metric``,`` `` target_performance ``=`` ``maximum_achievable_performance`` ``-`` ``0.02``,`` `` mean_or_assurance ``=`` ``"mean"``,`` `` test_n ``=`` ``500``,`` `` min_sample_size ``=`` ``50``,`` `` max_sample_size ``=`` ``300``,`` `` n_reps_total ``=`` ``20``,`` `` n_reps_per ``=`` ``5``,`` `` method ``=`` ``"bisection"``,`` `` progress ``=`` ``FALSE`` ``)`
