...
[`library`](https://rdrr.io/r/base/library.html)`(`[`pmsims`](https://pmsims-package.github.io/pmsims/)`)`` `` `[`set.seed`](https://rdrr.io/r/base/Random.html)`(``123``)`` `` ``binary_example`` ``<-`` `[`simulate_binary`](https://pmsims-package.github.io/pmsims/dev/reference/simulate_binary.md)`(`` `` signal_parameters ``=`` ``15``,`` `` noise_parameters ``=`` ``0``,`` `` predictor_type ``=`` ``"continuous"``,`` `` outcome_prevalence ``=`` ``0.20``,`` `` maximum_achievable_cstatistic ``=`` ``0.80``,`` `` model ``=`` ``"glm"``,`` `` metric ``=`` ``"calibration_slope"``,`` `` target_performance ``=`` ``0.90``,`` `` n_reps_total ``=`` ``1000``,`` `` mean_or_assurance ``=`` ``"assurance"`` ``)`
