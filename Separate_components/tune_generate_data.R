...
optimise_me <- function(tune_var, 
                        ...
                        performance_function, 
                        target_large_sample_performance) {
  data <- data_generating_function(large_n, tune_var)
  model <- model_function(data)
  test_data <- data_generating_function(large_n, tune_var)
  performance <- get_performance(data = test_data, model = model)
