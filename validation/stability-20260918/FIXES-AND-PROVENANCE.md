...
- The absolute plateau tolerance floor remains 0.005. This is twice the magnitude of the CSSE target for slope 0.95 (-0.0025). A controlled monotone curve with gains of 0.003 stops on a plateau at N=80 even though it crosses later at N=640; the returned upper N=160 is still below target. This is a retained limitation, not a newly introduced defect.
- The unchanged mlpwr final-selection rule permits a below-target GP mean through an optimistic uncertainty allowance and soft penalty. [The current GP review](GP-REVIEW.md) reproduces this in the 15-predictor run; independent checks remain necessary.
...
A read-only Air check also flags R/start_values.R and R/simulate_custom.R for formatting; [ridwan-format-check.txt](ridwan-format-check.txt) records that upstream check.
...
| fix/calibration-slope-search-instability at 7a39ab7 | [Calibration search study and seed reproduction](history/calibration-slope-search/README.md) | Broader proposed repair, empirical percentiles for small assurance batches, independent confirmation, and original-code seed-instability reproduction. Its production changes are not applied here. |
